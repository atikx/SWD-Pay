      ...
      if (newQuestion) {
        try {
          ...
          if (response.status === 200) {
            toast(response.data, toastConfig);
            setNewQuestion("");
            getQuestions();
          }
        } catch (error) {
          toast("Question cannot be asked now", toastConfig);
        }
      } else {
        toast("Question cannot be empty!", toastConfig);
      }
    };
    ...
    const handleAnswerQuestion = async (id) => {
      ...
      if (answer) {
        try {
          ...
          toast("Question cannot be answered now", toastConfig);
        }
      }
    };
